...
def plot_figs(output_folder, imagename, figs, img_index, signs, figsize=(8, 5), default_cmap='bwr', **kwargs):
    ...
    for fi in range(len(figs)):
        ...
        plt.savefig(os.path.join(output_folder, f'{signs[fi]}_{imagename}_{img_index}'), format='png', dpi=300)
        plt.close()
        print(f'Saved {signs[fi]}_{imagename}_{img_index}')
