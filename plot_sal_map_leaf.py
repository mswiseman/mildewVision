...
parser.add_argument('--dpi', type=str, required=True, help='inoculation date')
...
model_string = opt.model_type + '_upth' + str(opt.up_threshold) + '_downth' + str(
...
META_COL_NAMES = ['timestamp', 'tray', 'filename', 'clear_patches', 'hyphal_patches', 'conidiophores_patch', 'severity_rate_patch'] + key

# List all trays
tray = opt.trays

# threshold = opt.sal_threshold
threshold = 0.2
default_cmap = 'Blues'
...
for trays in tray:
    ...
    for leaf_disk_image_filename in leaf_disk_image_filenames:
        ...
        for _ in range(subim_y):
            for _ in range(subim_x):
                ...
                if not on_focus(subim_mask):
                    ...
                    if opt.save_conidiophores and logits_class == 2: 
        ...
        patch_info = {'infected_patch': infected_patch, 'conidiophores_patch': conidiophores_patch, 'clear_patch': clear_patch,
        ...
        if outdim == 3:
            ...
            plt.text(100, 370, f'Healthy Patches: {clear_patch}', color='white', fontsize=10)
            plt.text(100, 710, f'Hyphal Patches: {infected_patch}', color='white', fontsize=10)
            plt.text(100, 1050, f'Conidiophore Patches: {conidiophores_patch}', color='white', fontsize=10)
            plt.text(100, 1390, f'Combined Infected: {count_combined}', color='white', fontsize=10)
        ...
        for i, th in enumerate(pixel_th):
            ...
            record_data = [image_timestamp, tray, imagename_text, clear_patch, infected_patch, conidiophores_patch, severity_rate_patch] + list(
